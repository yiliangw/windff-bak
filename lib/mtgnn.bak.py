    ...
    def forward(self, g, x):
        with g.local_scope():
            ...
            for i in range(self.gdep):
                h = self.alpha * x + (1 - self.alpha) * self.nconv(g, h)
...
class MTGNN(nn.Module):
    def __init__(self, config, adj_mx=None):
        ...
        self.predefined_A = torch.tensor(self.adj_mx.astype(np.float32)) - torch.eye(
            self.num_nodes
        )
    ...
    def forward(self, batch_x, batch_y, data_mean, data_scale):
        ...
        if self.data_diff:
            ...
            inputs_diff = torch.cat(
                (torch.zeros(bz, id_len, 1, 1).to(inputs.device), inputs_diff), 2
            )
