...
def data_augment(X, y, p=0.8, alpha=0.5, beta=0.5):
    ...
    X[idx_to_change] += (1 - random_betas[idx_to_change]) * X[index_permute][
        idx_to_change
    ]

    y[idx_to_change] = random_betas[idx_to_change] * y[idx_to_change]
    y[idx_to_change] += (1 - random_betas[idx_to_change]) * y[index_permute][
        idx_to_change
    ]
...
def build_optimizer(config, log, model):
    ...
    if config.learner.lower() == "adam":
        ...
        optimizer = torch.optim.Adam(
            config.model.parameters(), lr=config.lr, weight_decay=config.weight_decay
        )
    return optimizer
...
def build_lr_scheduler(config, log, optimizer):
    ...
    if config.lr_decay:
        log.info(
            "You select `{}` lr_scheduler.".format(config.lr_scheduler_type.lower())
        )
        if config.lr_scheduler_type.lower() == "multisteplr":
            ...
            log.warning(
                "Received unrecognized lr_scheduler, please check the parameter `lr_scheduler`."
            )
            lr_scheduler = None
    else:
        lr_scheduler = None
...
def train_and_evaluate(config, train_data, valid_data, test_data=None):
    ...
    data_mean = torch.FloatTensor(train_data.data_mean).to(
        config.device
    )  # (1, 134, 1, 1)
    data_scale = torch.FloatTensor(train_data.data_scale).to(
        config.device
    )  # (1, 134, 1, 1)
    ...
    for epoch in range(config.epoch):
        ...
        log.info(
            "Epoch=%s, exp_id=%s, Train Loss: %s"
            % (epoch, config.exp_id, np.mean(losses))
        )
        ...
        log.info(
            "Epoch={}, exp_id={}, Valid ".format(epoch, config.exp_id)
            + str(dict(valid_r))
        )
        ...
        if best_score == valid_r["score"]:
            patient = 0
            save_model(
                config.output_path + config.exp_id, model, opt=opt, steps=epoch, log=log
            )
        else:
            patient += 1
...
def evaluate(
    ...
    with torch.no_grad():
        ...
        for batch_x, batch_y in tqdm(valid_data_loader, tag):
            ...
            scaled_batch_y = (scaled_batch_y - data_mean[:, :, :, -1]) / data_scale[
                :, :, :, -1
            ]
        ...
        _mae, _rmse = regressor_detailed_scores(
            pred_batch, gold_batch, valid_raw_df, config.capacity, config.output_len
        )

        _farm_mae, _farm_rmse = regressor_scores(
            np.sum(pred_batch, 0) / 1000.0, np.sum(gold_batch, 0) / 1000.0
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--random", type=str2bool, default=False, help="Whether shuffle num_nodes"
    )
    parser.add_argument(
        "--enhance", type=str2bool, default=True, help="Whether enhance the time dim"
    )
    parser.add_argument(
        "--only_useful", type=str2bool, default=True, help="Whether remove some feature"
    )
    parser.add_argument(
        "--var_len", type=int, default=5, help="Dimensionality of input features"
    )
    parser.add_argument(
        "--data_diff",
        type=str2bool,
        default=False,
        help="Whether to use data differential features",
    )
    parser.add_argument(
        "--add_apt", type=str2bool, default=False, help="Whether to use adaptive matrix"
    )
    parser.add_argument(
        "--binary",
        type=str2bool,
        default=True,
        help="Whether to set the adjacency matrix as binary",
    )
    parser.add_argument(
        "--graph_type", type=str, default="geo", help="graph type, dtw or geo"
    )
    parser.add_argument("--dtw_topk", type=int, default=5, help="M dtw for dtw graph")
    parser.add_argument(
        "--weight_adj_epsilon", type=float, default=0.8, help="epsilon for geo graph"
    )
